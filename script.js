...
class Particle {
    ...
    draw(ctx) { ctx.fillStyle = `rgba(255, 0, 85, ${this.life})`; ctx.beginPath(); ctx.arc(this.pos.x, this.pos.y, 2, 0, Math.PI*2); ctx.fill(); }
}
...
class Boid {
    ...
    flock(boids, config, foods, obstacles, predators) {
        ...
        predators.forEach(p => { if(Vector.dist(this.position, p.position) < 100) this.applyForce(this.position.clone().sub(p.position).setMag(config.maxSpeed).limit(config.maxForce * 4)); });
        foods.forEach((f, i) => { if(Vector.dist(this.position, f) < 15) { foods.splice(i, 1); updateScore(10); } else if(Vector.dist(this.position, f) < 100) this.applyForce(f.clone().sub(this.position).setMag(config.maxSpeed).limit(config.maxForce*2)); });
        obstacles.forEach((o, i) => { if(Vector.dist(this.position, o) < o.r + 30) { this.applyForce(this.position.clone().sub(new Vector(o.x, o.y)).setMag(config.maxSpeed).limit(config.maxForce*4)); if(Vector.dist(this.position, o) < o.r+5) { o.health -= 2; if(o.health <= 0) { obstacles.splice(i,1); updateScore(5); for(let k=0; k<8; k++) pEffects.push(new Particle(o.x, o.y)); } } } });
    }
    draw(ctx) {
        ...
        ctx.beginPath(); ctx.moveTo(this.size*3, 0); ctx.lineTo(-this.size, this.size); ctx.lineTo(-this.size, -this.size); ctx.fill(); ctx.restore();
    }
}

class Predator extends Boid {
    constructor(x, y) { super(x, y); this.color = "#ff0055"; this.size = 6; }
    hunt(boids) {
        let closest = null, minDist = 300;
        boids.forEach(b => { let d = Vector.dist(this.position, b.position); if(d < minDist) { minDist = d; closest = b; } });
        if (closest) {
            this.applyForce(closest.position.clone().sub(this.position).setMag(3.5).limit(0.1));
            if (minDist < 12) { boids.splice(boids.indexOf(closest), 1); updateScore(-20); for(let k=0; k<10; k++) pEffects.push(new Particle(closest.position.x, closest.position.y)); }
        }
    }
}
...
function init() {
    ...
    predators = [new Predator(100, 100), new Predator(canvas.width-100, canvas.height-100)];
    document.getElementById('high-score').innerText = highScore;
}

function updateScore(p) { score += p; document.getElementById('current-score').innerText = score; if (score > highScore) { highScore = score; localStorage.setItem('boids_pro_hs', highScore); document.getElementById('high-score').innerText = highScore; } }

function drawGrid() {
    ctx.strokeStyle = 'rgba(0, 255, 136, 0.03)'; ctx.lineWidth = 0.5;
...
function animate() {
    ...
    if (isStormy) {
        ...
        ctx.strokeStyle = 'rgba(0, 255, 255, 0.05)'; ctx.beginPath(); ctx.arc(center.x, center.y, 150, 0, Math.PI*2); ctx.stroke();
    }

    foods.forEach(f => { ctx.shadowBlur = 15; ctx.shadowColor = "#fffb00"; ctx.fillStyle = "#fffb00"; ctx.beginPath(); ctx.arc(f.x, f.y, 4, 0, Math.PI*2); ctx.fill(); });
    obstacles.forEach(o => { ctx.shadowBlur = 10; ctx.shadowColor = "#00ffff"; ctx.strokeStyle = `rgba(0, 255, 255, ${o.health/100})`; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(o.x, o.y, o.r, 0, Math.PI*2); ctx.stroke(); });
...
// UI & Events
window.addEventListener('load', () => setTimeout(() => { document.getElementById('loader').style.display='none'; document.getElementById('help-modal').style.display='block'; }, 2000));
...
const getPos = (e) => ({ x: (e.touches?e.touches[0]:e).clientX, y: (e.touches?e.touches[0]:e).clientY });
...
canvas.addEventListener('touchstart', (e) => { isDrawing=true; let p=getPos(e); if(currentTool==='food') foods.push(new Vector(p.x, p.y)); e.preventDefault(); }, {passive:false});
canvas.addEventListener('touchmove', (e) => { if(isDrawing && currentTool==='wall') { let p=getPos(e); obstacles.push({x:p.x, y:p.y, r:15, health:100}); } e.preventDefault(); }, {passive:false});
